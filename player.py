...
class Player:
	...
	def __init__(self, WIDTH, HEIGHT):
		...
		self.img_swim = pygame.transform.scale(pygame.image.load('Assets/temp_player.png').convert_alpha(),(Player.PLAYER_WIDTH,Player.PLAYER_HEIGHT))
		self.img_hit = pygame.transform.scale(pygame.image.load('Assets/temp_player_hit.png').convert_alpha(),(Player.PLAYER_WIDTH,Player.PLAYER_HEIGHT))
		self.img_bounce = pygame.transform.scale(pygame.image.load('Assets/temp_player_bounce.png').convert_alpha(),(Player.PLAYER_WIDTH,Player.PLAYER_HEIGHT))
		...
		if self.state == 'BOUNCE':
			...
			if delta >= self.bounce_time:
				...
				if self.check_rebound_snorkle(snorklers):
					print("WE HIT A SNORKLER")
					self.state = 'HIT'
					self.hit_timer = pygame.time.get_ticks()
				else:
					print("WE DIDN'T HIT A SNORKLER")
					self.state = 'SWIMMING'
	...
	def snorkle_hit_check(self, snorkles):
		for snorkle in snorkles:
			if pygame.Rect.colliderect(self.rect, snorkle.rect) and not snorkle.used and self.get_distance(snorkle) < 25:
				self.state = 'BOUNCE'
				self.bounce_timer = pygame.time.get_ticks()
				snorkle.activated()
	...
	def get_distance(self, obj):
		x_dif = (self.rect.x + Player.PLAYER_WIDTH/2) - (obj.rect.x + obj.SNORKLE_WIDTH/2)
		y_dif = (self.rect.y + Player.PLAYER_HEIGHT/2) - (obj.rect.y + obj.SNORKLE_HEIGHT/2)
